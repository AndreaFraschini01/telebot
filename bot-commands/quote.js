// Comando che permette di salvare le citazioni nel database

module.exports = (bot, db) => {
	bot.command("quote", (ctx) => {
		if (ctx.message.reply_to_message) {
			//Controlla che il messaggio sia una risposta
			...
			if (text == undefined) {
				...
				db.inserisciCitazione(idChat, { text, author, date: data }, function (
					upserted,
					modified
				) {
					if (modified == 0 && upserted == 0) {
						//se non ci sono record aggiunti vuol dire che era già stata inserita
			...
			ctx.reply("Non vedo il messaggio da salvare!");
		}
	});
};
